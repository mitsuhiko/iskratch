var iskratch = {
  ...
  getNextID : function() {
    return ++iskratch._lastID;  
  }
};

iskratch.canvasTools = {
...
iskratch.Sprite = Class.$extend({
  __init__ : function(x, y) {
    this.id = iskratch.getNextID();
    this.costumes = [];
  ...
  nextCostume : function() {
    this.activeCostume = (this.activeCostume + 1) % 2;
  },
...
iskratch.Application = Class.$extend({
  FRAMERATE : 30,
  ...
  stop : function() {
    if (this.render_interval === null)
      return;
    window.clearInterval(this.render_interval);
    this.render_interval = null;
  ...
  renderLoop : function(rt) {
    this.render_interval = window.setInterval(function() {
  ...
  run : function() {
    ...
    if (this.render_interval !== null)
      this.stop();
    var rt = new iskratch.Runtime(this);
...
iskratch.Runtime = Class.$extend({
  ...
  drawBubble : function(x, y, r, fillStyle, text) {
    ...
    iskratch.canvasTools.traceRoundedRect(this.ctx, x, y, metric.width + 20, 30, r);
    this.ctx.fill();
    this.ctx.stroke();
  ...
  drawThinkBubble : function(x, y, text) {
    ...
    iskratch.canvasTools.traceEllipse(this.ctx, x + 10, y + 33, 20, 10);
    this.ctx.fillStyle = fillStyle;
    this.ctx.fill();
    this.ctx.stroke();
    iskratch.canvasTools.traceEllipse(this.ctx, x + 20, y + 45, 15, 8);
    this.ctx.fill();
    this.ctx.stroke();
  },
  ...
  handleClick : function(evt) {
    ...
    for (var i = this.sprites.length - 1, i; i != n; --i) {
  ...
  wait : function(timeout, func) {
    if (this.app.running)
      window.setTimeout(func, timeout * 1000);
  },
